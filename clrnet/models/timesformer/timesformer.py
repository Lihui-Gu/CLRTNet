...
from clrnet.models.registry import TIMESFORMER
...
class TimeSformer(nn.Module):
    def __init__(
        ...
        # embedding 层设计
        self.to_patch_embedding_list = nn.ModuleList([])
        patch_dim_list = [c * patch_size ** 2 for c in channels]
        for patch_dim in patch_dim_list:
            self.to_patch_embedding_list.append(nn.Linear(patch_dim, dim))
    ...
    def forward(self, video_list, mask=None):
        fx = [8, 4, 2]
        result_list = []
        for i, patch_embedding in enumerate(self.to_patch_embedding_list):
            video = rearrange(video_list[i + 1], '(b f) c w h -> b f c w h', f=1)  # batch_size
            b, f, _, h, w, *_, device, p = *video.shape, video.device, self.patch_size
            # mask = torch.ones(b, f).bool().cuda()
            # 每 f 张图片为1次batch
            ...
            for (time_attn, spatial_attn, ff) in self.layers:
                x = time_attn(x, 'b (f n) d', '(b n) f d', n=n, mask=frame_mask, cls_mask=cls_attn_mask,
                              rot_emb=frame_pos_emb) + x
                x = spatial_attn(x, 'b (f n) d', '(b f) n d', f=f, cls_mask=cls_attn_mask, rot_emb=image_pos_emb) + x
                x = ff(x) + x

            result = x[:, 1:, :]
            result = rearrange(result, 'b (f n) d -> b d f n', f=fx[i])
